...
**No Lombok.** All entities, DTOs, and services use plain manual getters/setters and field-level `@Autowired`
injection (same style as a typical Eclipse/STS project). This avoids the whole class of "method X is undefined"
IDE errors that come from Lombok's annotation processor not being wired into STS correctly — one less thing to
configure while you're learning the codebase.
